    ...
    try {
        ...
        try {
            ...
            const [student] = await conn.query(
                `SELECT id, payment_status FROM students WHERE id = ?`, [student_id]
            );
            ...
            const [course] = await conn.query(
                `SELECT id, name, status FROM courses WHERE id = ? AND status = 'ACTIVE'`, [course_id]
            );
            ...
            const [existing] = await conn.query(
                `SELECT id FROM invoices WHERE student_id = ? AND course_id = ? AND status IN ('PENDING', 'PAID')`, [student_id, course_id]
            );
         ...
         VALUES (?, ?, ?, 'PENDING', NOW())`, [student_id, course_id, parseFloat(amount)]
            );
            ...
            if (student[0].payment_status === 'NEW') {
                await conn.query(
                    `UPDATE students SET payment_status = 'PENDING' WHERE id = ?`, [student_id]
                );
            }
    ...
    try {
        ...
        try {
            ...
            const [invoice] = await conn.query(
                `SELECT id FROM invoices WHERE student_id = ? AND course_id = ? AND status = 'PAID'`, [student_id, course_id]
            );
            ...
            const [existing] = await conn.query(
                `SELECT id FROM class_students WHERE class_id = ? AND student_id = ? AND status = 'ACTIVE'`, [class_id, student_id]
            );
         ...
         VALUES (?, ?, 'ACTIVE', NOW())`, [class_id, student_id]
            );
    ...
    try {
        ...
        try {
            ...
            const [classStudent] = await conn.query(
                `SELECT id, status FROM class_students WHERE class_id = ? AND student_id = ?`, [class_id, student_id]
            );
         ...
         WHERE class_id = ? AND student_id = ?`, [class_id, student_id]
            ...
            await conn.query(
                `UPDATE students SET payment_status = 'PAID' WHERE id = ?`, [student_id]
            );
...
router.delete('/courses/:id', async(req, res) => {
    try {
        ...
        try {
         ...
         WHERE i.course_id = ? AND i.status = 'PAID' AND s.payment_status = 'PAID'`, [course_id]
            );
            ...
            if (!paidStudents || paidStudents.length === 0) {
                ...
                await conn.query(
                    `DELETE FROM invoices WHERE course_id = ?`, [course_id]
                );
...
router.get('/bank-info', async(req, res) => {
    try {
        ...
        const [bank] = await conn.query(
            `SELECT id, bank_name, account_number, account_holder, qr_code 
       FROM bank_info 
       ...
       LIMIT 1`
        ...
        console.error('Error in GET /bank-info:', err);
        res.status(500).json({ success: false, message: 'Lỗi server', error: err.message });
    }
});

// ============================================
// 12. GET /api/fee/report/summary
...
router.get('/report/summary', async(req, res) => {
    try {
      ...
      SELECT
        ...
        IFNULL(COUNT(DISTINCT i.student_id), 0) AS students_registered
      FROM invoices i
      ${where}
    `;
