...
router.post('/classes/:id/sessions', async(req, res) => {
    ...
    try {
        ...
        const [
            [cls]
        ] = await conn.query('SELECT id FROM classes WHERE id = ?', [classId]);
        ...
        const [result] = await conn.query(
            'INSERT INTO class_sessions (class_id, date, time_start, time_end, room, teacher_id, meta) VALUES (?, ?, ?, ?, ?, ?, ?)', [classId, date, time_start, time_end, room || null, teacher_id || null, meta ? JSON.stringify(meta) : null]
        );
...
router.put('/classes/:id/sessions/:sessionId', async(req, res) => {
    ...
    try {
        ...
        const [
            [sess]
        ] = await conn.query('SELECT * FROM class_sessions WHERE id = ? AND class_id = ?', [sessionId, classId]);
...
router.get('/sessions', async(req, res) => {
    ...
    try {
        const clauses = [];
        const params = [];
        if (dateFrom) { clauses.push('date >= ?');
            params.push(dateFrom); }
        if (dateTo) { clauses.push('date <= ?');
            params.push(dateTo); }
        if (classId) { clauses.push('class_id = ?');
            params.push(classId); }
        if (teacherId) { clauses.push('teacher_id = ?');
