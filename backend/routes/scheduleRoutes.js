...
router.post('/finish', async(req, res) => {
    ...
    try {
        ...
        // Đánh dấu tất cả schedules của học viên này là FINISHED
        await db.query('UPDATE schedules SET status = ?, finished_at = NOW() WHERE student_id = ? AND (status IS NULL OR status != ?)', ['FINISHED', studentId, 'FINISHED']);

        await db.query('UPDATE students SET status = ?, updated_at = NOW() WHERE id = ?', ['COMPLETED', studentId]);

        return res.json({ success: true, message: 'Student marked COMPLETED and schedules finished' });
    } catch (err) {
