...
// Dùng cho: "+ Thêm học viên mới & cấp tài khoản"
...
// Dùng cho: khối "Tìm kiếm & cập nhật thông tin học viên"
...
// Lấy lịch học hiện tại của 1 học viên theo username
export async function getStudentSchedule(username) {
    if (!username) {
    ...
    if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(
            data.message || "Không lấy được quá trình học hiện tại của học viên"
        );
    }
