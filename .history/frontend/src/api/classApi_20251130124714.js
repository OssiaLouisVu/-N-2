...
export async function createSchedulesForClass(classId, studentId, sessionDates, sessionTimeStart, sessionTimeEnd) {
    const body = { studentId };
    if (Array.isArray(sessionDates) && sessionDates.length > 0) body.sessionDates = sessionDates;
...
export async function getSchedules(query = {}) {
    const params = new URLSearchParams();
    Object.keys(query).forEach(k => { if (query[k] != null && query[k] !== '') params.append(k, query[k]); });
    // backend route exposed as /api/classes/schedules
    const res = await fetch(`${API_BASE}/api/classes/schedules?${params.toString()}`);
    if (!res.ok) throw new Error(`HTTP error ${res.status}`);
    return res.json();
}

export async function updateSchedule(scheduleId, payload) {
    const res = await fetch(`${API_BASE}/api/classes/schedules/${scheduleId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
    });
...
export async function deleteSchedule(scheduleId) {
    const res = await fetch(`${API_BASE}/api/classes/schedules/${scheduleId}`, { method: 'DELETE' });
    return res.json();
}
