...
export default function TeacherDashboard() {
  ...
  // Load danh sách lớp từ API - phân loại theo status
  useEffect(() => {
    const u = stored.username;
    fetch(`http://localhost:8080/api/teacher/${u}/classes`)
      ...
      .then((data) => {
        if (data.success && data.classes) {
          // Phân loại theo status: UPCOMING, ACTIVE, COMPLETED
          const upcomingList = data.classes.filter((c) => c.status === "UPCOMING");
          const ongoingList = data.classes.filter((c) => c.status === "ACTIVE");
          const finishedList = data.classes.filter((c) => c.status === "COMPLETED");
                        ...
                        {filterList(upcoming).length === 0 ? (
                          ...
                          filterList(upcoming).map((c) => (
                            <tr key={c.id}>
                              ...
                              <td style={tdStyle}>{new Date(c.startDate).toLocaleDateString('vi-VN')}</td>
                              <td style={tdStyle}>{new Date(c.endDate).toLocaleDateString('vi-VN')}</td>
