...
export default function FeeInquiry() {
  ...
  const getFilteredInvoices = () => {
    ...
    // Filter by course
    if (filterCourse) {
      filtered = filtered.filter(inv => inv.course_id === parseInt(filterCourse));
    }
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
        ...
        {selectedStudent && (
          <div style={{ background: '#fff', padding: 20, borderRadius: 12, marginBottom: 30, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 15, alignItems: 'end' }}>
              <div>
              ...
              <div>
                <label style={{ display: 'block', fontWeight: 600, marginBottom: 5, fontSize: 13 }}>Khoá Học</label>
                <select 
                  value={filterCourse} 
                ...
                >
                  ...
                  {[...new Set(invoices.map(inv => inv.course_id))].map(courseId => {
                    const courseName = invoices.find(inv => inv.course_id === courseId)?.course_name;
                    return <option key={courseId} value={courseId}>{courseName}</option>;
                  })}
                </select>
              </div>
              <div>
