...
export default function FeeManagement() {
  ...
  if (!stored) {
    ...
    window.location.href = '/login';export default function FeeManagement() {
  ...
  const [loading, setLoading] = useState(true);  if (!stored) {

  const [selectedStudent, setSelectedStudent] = useState(null);

  const [formCreate, setFormCreate] = useState({ course_id: '', amount: '' });    window.location.href = '/login';export default function FeeManagement() {export default function FeeManagement() {

  const [selectedInvoice, setSelectedInvoice] = useState(null);

  const [formPayment, setFormPayment] = useState({ method: 'cash', note: '' });    return null;

  const [filterStatus, setFilterStatus] = useState('all');

  const [searchText, setSearchText] = useState('');  }  const navigate = useNavigate();  const navigate = useNavigate();
  ...
  useEffect(() => {  const [tab, setTab] = useState('create');  const stored = JSON.parse(localStorage.getItem('currentUser'));  const stored = JSON.parse(localStorage.getItem('currentUser'));

    loadData();

  }, [tab]);  const [students, setStudents] = useState([]);



  const loadData = async () => {  const [courses, setCourses] = useState([]);

    setLoading(true);

    setMessage('');  const [invoices, setInvoices] = useState([]);

    try {

      if (tab === 'create') {  const [loading, setLoading] = useState(true);  if (!stored) {  if (!stored) {

        const res1 = await fetch(`${API_BASE}/fee/students/new`).then(r => r.json());

        const res2 = await fetch(`${API_BASE}/fee/courses/active`).then(r => r.json());  const [selectedStudent, setSelectedStudent] = useState(null);

        setStudents(res1.students || []);

        setCourses(res2.courses || []);  const [formCreate, setFormCreate] = useState({ course_id: '', amount: '' });    window.location.href = '/login';    window.location.href = '/login';
      ...
      } else if (tab === 'list') {  const [formPayment, setFormPayment] = useState({ method: 'cash', note: '' });    return null;    return null;

        const res = await fetch(`${API_BASE}/fee/invoices/all`).then(r => r.json());

        setInvoices(res.invoices || []);  const [filterStatus, setFilterStatus] = useState('all');

      }

    } catch (err) {  const [searchText, setSearchText] = useState('');  }  }

      setMessage('❌ Lỗi tải dữ liệu: ' + err.message);

      console.error(err);  const [message, setMessage] = useState('');

    }

    setLoading(false);  const [submitting, setSubmitting] = useState(false);
  ...
  const handleCreateInvoice = async (e) => {
    ...
    if (!selectedStudent || !formCreate.course_id || !formCreate.amount) {
      ...
      setMessage('⚠️ Vui lòng chọn học viên, khoá học và nhập số tiền');    fetchData();
    ...
    try {    setLoading(true);
      ...
      const res = await fetch(`${API_BASE}/fee/invoices`, {
        ...
        method: 'POST',    setMessage('');  const [invoices, setInvoices] = useState([]);  const [courses, setCourses] = useState([]);
        ...
        body: JSON.stringify({    try {
          ...
          course_id: parseInt(formCreate.course_id),      if (tab === 'create') {  const [loading, setLoading] = useState(true);  const [loading, setLoading] = useState(true);
      ...
      if (res.success) {          fetch(`${API_BASE}/fee/students/new`).then(r => r.json()),

        setMessage('✅ Tạo hoá đơn thành công! (Trạng thái: PENDING)');

        setFormCreate({ course_id: '', amount: '' });          fetch(`${API_BASE}/fee/courses/active`).then(r => r.json()),

        setSelectedStudent(null);

        setTimeout(() => loadData(), 1000);        ]);  // Form tạo hoá đơn  // Form tạo hoá đơn

      } else {

        setMessage('❌ Lỗi: ' + (res.message || 'Không xác định'));        setStudents(studentsRes.students || []);

      }

    } catch (err) {        setCourses(coursesRes.courses || []);  const [selectedStudent, setSelectedStudent] = useState(null);  const [selectedStudent, setSelectedStudent] = useState(null);

      setMessage('❌ Lỗi: ' + err.message);

      console.error(err);      } else if (tab === 'payment') {

    }

    setSubmitting(false);        const invoicesRes = await fetch(`${API_BASE}/fee/invoices/pending`).then(r => r.json());  const [formCreate, setFormCreate] = useState({  const [formData, setFormData] = useState({
  ...
  const handlePayment = async (e) => {
    ...
    e.preventDefault();      } else if (tab === 'list') {    course_id: '',    course_id: '',

    if (!selectedInvoice) {

      setMessage('⚠️ Vui lòng chọn hoá đơn');        const invoicesRes = await fetch(`${API_BASE}/fee/invoices/all`).then(r => r.json());
    ...
    try {      }
      ...
      const res = await fetch(`${API_BASE}/fee/invoices/${selectedInvoice.id}/payment`, {
        ...
        method: 'PUT',    } catch (err) {  });    method: 'cash',

        headers: { 'Content-Type': 'application/json' },

        body: JSON.stringify({      setMessage('❌ Lỗi tải dữ liệu: ' + err.message);

          method: formPayment.method,

          note: formPayment.note,      console.error(err);    note: ''
      ...
      if (res.success) {
        ...
        setMessage('✅ Thanh toán thành công! Học viên chuyển sang PAID');    setLoading(false);  // Form thanh toán  });
      ...
      setMessage('❌ Lỗi: ' + err.message);    e.preventDefault();  const [formPayment, setFormPayment] = useState({  const [submitting, setSubmitting] = useState(false);
  ...
  return (
    ...
    <div style={{      setMessage('⚠️ Số tiền phải lớn hơn 0');  });    fetchData();

      width: '100%',

      minHeight: '100vh',      return;
      ...
      <div style={{    setSubmitting(true);

        background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',

        color: '#fff',    try {  // Tab 3 filter + search
