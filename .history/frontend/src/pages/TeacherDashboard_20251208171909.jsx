    ...
    fetch(`http://localhost:8080/api/teacher/${u}/classes`)
      ...
      .then((data) => {
        if (data.success && data.classes) {
          // Phân loại đúng status từ backend
          setUpcoming(data.classes.filter((c) => c.status === "UPCOMING"));
          setOngoing(data.classes.filter((c) => c.status === "ACTIVE"));
          setFinished(data.classes.filter((c) => c.status === "COMPLETED"));
        }
      })
      .catch((err) => console.error(err));
  }, []);
