...
export default function StudentDashboard() {
  ...
  // Listen to global event so staff actions can notify student dashboard to refresh
  useEffect(() => {
    const handler = (e) => {
      // If the schedule panel is open, refresh immediately.
      // We intentionally do not attempt to map studentId -> username here; any update will trigger a refresh.
      if (activeFeature === "schedule") {
        loadSchedule();
      }
    };
    window.addEventListener('studentScheduleUpdated', handler);
    return () => window.removeEventListener('studentScheduleUpdated', handler);
  }, [activeFeature]);

  const loadSchedule = async () => {
    setLoading(true);
    try {
  const res = await fetch(`${API_BASE}/api/students/${username}/schedule?latestOnly=true&limit=5`);
            ...
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                <button
                  ...
                  style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #ddd", background: "#f5f5f5", cursor: "pointer" }}
                >
                  Làm mới
                </button>
                <span style={{ color: "#666" }}>Chỉ hiển thị 5 buổi sắp tới</span>
              </div>
              {schedule.length === 0 ? (
...
<tbody>
  {schedule
    .filter(item => {
      // Ẩn lịch nếu status là FINISHED
      if (item.status === 'FINISHED') return false;
      // Ẩn lịch nếu đã quá ngày kết thúc khoá (nếu có endDate)
      if (item.classEndDate) {
        const end = new Date(item.classEndDate);
        const d = new Date(item.date);
    ...
    .map((item, index) => {
      const dateStr = formatVNDateDash(item.date);
