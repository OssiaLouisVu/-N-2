...
import { fetchEmployees as apiFetchEmployees, addEmployee as apiAddEmployee, deactivateEmployee as apiDeactivateEmployee, sendNotification as apiSendNotification } from "../api/employeeApi";
...
export default function ManagerDashboard() {
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      let data;
      if (editId) {
        data = await window.imports.updateEmployee(editId, { ...form, dob });
      } else {
  ...
  async function loadAttendance() {
    ...
    try {
      ...
      const activeRes = await fetch(`${API_BASE}/api/employees?active=true`);
            ...
            {showAdd && (
              <form className="mt-6 bg-gray-50 p-4 rounded-xl shadow-card" onSubmit={handleAdd}>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="btn-outline" type="button" onClick={() => { setShowAdd(false); setEditId(null); setForm({ fullName: '', dob: '', gender: '', phone: '', email: '', address: '' }); }}>Huỷ</button>
                </div>
              </form>
            )}
          </>
