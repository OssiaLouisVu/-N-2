...
import { fetchEmployees as apiFetchEmployees, addEmployee as apiAddEmployee, deactivateEmployee as apiDeactivateEmployee, sendNotification as apiSendNotification, updateEmployee as apiUpdateEmployee } from "../api/employeeApi";
              ...
              <div className="ml-auto flex gap-2 items-center">
                ...
                <button
                  ...
                  onClick={async()=>{
                    ...
                    if(attendanceItems.length===0){ setAttendanceMessage('Hãy tải danh sách và lưu trước khi gửi'); return; }
                    // Chọn danh sách theo sendTarget
                    const itemsToSend = attendanceItems.filter(it => sendTarget==='all' ? true : it.status === sendTarget);
                    if(itemsToSend.length===0){ setAttendanceMessage('Không có nhân viên phù hợp để gửi'); return; }
                    try{
                      // Gửi email từng người để cá nhân hoá nội dung
                      for(const it of itemsToSend){
                        const vnStatus = it.status==='present' ? 'Đi làm' : it.status==='leave' ? 'Nghỉ phép' : it.status==='sick' ? 'Nghỉ ốm' : 'Vắng';
                        const d = new Date(attendanceDate);
                        const dd = String(d.getDate()).padStart(2,'0');
                        const mm = String(d.getMonth()+1).padStart(2,'0');
                        const yyyy = d.getFullYear();
                        const title = `[Chấm công] Ngày ${dd}/${mm}/${yyyy}`;
                        const content = `Xin chào ${it.full_name},\n\nTrạng thái chấm công ngày ${dd}/${mm}/${yyyy} của bạn: ${vnStatus}.\nGhi chú: ${it.note ? it.note : 'Không có'}.\n\nNếu có sai sót, vui lòng phản hồi quản lý.`;
                        // Gọi API gửi cho một người mỗi lần để nội dung riêng biệt
                        // eslint-disable-next-line no-await-in-loop
                        await apiSendNotification({ employeeIds: [it.employee_id], title, content });
                      }
                      setAttendanceMessage(`Đã gửi email cho ${itemsToSend.length} nhân viên`);
                    }catch(err){
