...
export default function TeacherDashboard() {
  ...
  // Load danh sách lớp 3 nhóm
  useEffect(() => {
    fetch("http://localhost:8080/api/teacher/classes/upcoming")
      .then((res) => res.json())
      .then((data) => setUpcoming(data.data || []))
      .catch((err) => console.error(err));

    fetch("http://localhost:8080/api/teacher/classes/ongoing")
      .then((res) => res.json())
      .then((data) => setOngoing(data.data || []))
      .catch((err) => console.error(err));

    fetch("http://localhost:8080/api/teacher/classes/finished")
      .then((res) => res.json())
      .then((data) => setFinished(data.data || []))
      .catch((err) => console.error(err));
  }, []);
          ...
          <div style={{ marginTop: 24, marginBottom: 24, clear: "both" }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <button
                ...
                className={showClasses ? 'btn-gradient-primary btn-active-glow' : 'btn-soft'}
              >{showClasses ? '📍 Đóng danh sách lớp' : '📚 Xem danh sách lớp học'}</button>

              {/* Nút điểm danh */}
              <button
                onClick={() => setShowAttendance(prev => !prev)}
                className={showAttendance ? 'btn-gradient-green btn-active-glow' : 'btn-soft'}
              >{showAttendance ? '✖️ Ẩn điểm danh' : '✏️ Điểm danh lớp học'}</button>

              {/* Nút xem danh sách điểm thi thử */}
              <button
                onClick={() => setShowMockExamScores(prev => !prev)}
                className={showMockExamScores ? 'btn-gradient-primary btn-active-glow' : 'btn-soft'}
              >{showMockExamScores ? '📊 Đóng danh sách điểm thi thử' : '📊 Xem danh sách điểm thi thử'}</button>
            </div>
          </div>
