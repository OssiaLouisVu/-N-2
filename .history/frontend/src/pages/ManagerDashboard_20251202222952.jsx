...
export default function ManagerDashboard() {
    ...
    try {
      ...
      setAttendanceMessage("Lỗi tải điểm danh");
        ...
        <div className="max-w-5xl mx-auto -mt-12 mb-8 bg-white rounded-2xl shadow-xl p-8 relative z-10">
          ...
          {tab === 'attendance' && (
            <div className="w-full">{/* ...existing code... */}</div>
          )}
          {tab === 'notify' && (
            <div>{/* ...existing code... */}</div>
          )}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header gradient giống teacher */}
      <div className="w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-400 py-10 px-6 md:px-0 flex flex-col items-center relative shadow-md">
        <div className="max-w-5xl w-full flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white drop-shadow mb-2">Dashboard Quản lý</h1>
            <div className="text-white text-lg font-medium">Xin chào, <span className="font-bold">{currentUser?.username || 'manager'}</span></div>
          </div>
          <button
            onClick={handleLogout}
            className="mt-6 md:mt-0 px-7 py-2 border border-white text-white rounded-full font-semibold text-base hover:bg-white hover:text-pink-500 transition shadow"
            style={{minWidth:120}}
          >
            Đăng xuất
          </button>
