...
export default function FeeManagement() {
  ...
  const navigate = useNavigate();const API_BASE = 'http://localhost:8080/api';const API_BASE = 'http://localhost:8080/api';
  ...
  if (!user) {export default function FeeManagement() {export default function FeeManagement() {

    navigate('/login');

    return null;  const navigate = useNavigate();  const navigate = useNavigate();
  ...
  const [students, setStudents] = useState([]);  const username = user?.username || 'Kế Toán';  const username = user?.username || 'Kế Toán';
  ...
  const [selectedStudent, setSelectedStudent] = useState(null);  if (!user) {  if (!user) {

  const [selectedInvoice, setSelectedInvoice] = useState(null);

  const [amount, setAmount] = useState('');    navigate('/login');    navigate('/login');

  const [courseId, setCourseId] = useState('');

  const [paymentMethod, setPaymentMethod] = useState('cash');    return null;    return null;
  ...
  useEffect(() => {  const [students, setStudents] = useState([]);  const [students, setStudents] = useState([]);

    loadTabData();

  }, [tab]);  const [courses, setCourses] = useState([]);  const [courses, setCourses] = useState([]);



  const loadTabData = async () => {  const [invoices, setInvoices] = useState([]);  const [invoices, setInvoices] = useState([]);

    setLoading(true);

    setMessage('');  const [loading, setLoading] = useState(false);  const [loading, setLoading] = useState(false);

    try {

      if (tab === 'create') {  const [selectedStudent, setSelectedStudent] = useState(null);  const [selectedStudent, setSelectedStudent] = useState(null);

        const s = await fetch(`${API_BASE}/fee/students/new`).then(r => r.json());

        const c = await fetch(`${API_BASE}/fee/courses/active`).then(r => r.json());  const [selectedInvoice, setSelectedInvoice] = useState(null);  const [selectedInvoice, setSelectedInvoice] = useState(null);

        setStudents(s.students || []);

        setCourses(c.courses || []);  const [amount, setAmount] = useState('');  const [amount, setAmount] = useState('');
        ...
        setInvoices(inv.invoices || []);  const [paymentMethod, setPaymentMethod] = useState('cash');  const [paymentMethod, setPaymentMethod] = useState('cash');

        if (bank.success && bank.bank) {

          setBankInfo(bank.bank);  const [paymentNote, setPaymentNote] = useState('');  const [paymentNote, setPaymentNote] = useState('');

        }

      } else if (tab === 'list') {  const [message, setMessage] = useState('');  const [message, setMessage] = useState('');
    ...
    } catch (e) {  const [filterStatus, setFilterStatus] = useState('all');  const [filterStatus, setFilterStatus] = useState('all');
  ...
  const createInvoice = async (e) => {
    ...
    if (!selectedStudent || !courseId || !amount) {  useEffect(() => {  useEffect(() => {
      ...
      return;    loadTabData();    loadTabData();

    }

    setSubmitting(true);  }, [tab]);  }, [tab]);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/fee/invoices`, {
        ...
        headers: { 'Content-Type': 'application/json' },  const loadTabData = async () => {  const loadTabData = async () => {
      ...
      if (res.success) {
        ...
        setMessage('✅ Tạo hoá đơn thành công');      if (tab === 'create') {      if (tab === 'create') {
        ...
        setTimeout(() => loadTabData(), 1000);        const c = await fetch(`${API_BASE}/fee/courses/active`).then(r => r.json());        const c = await fetch(`${API_BASE}/fee/courses/active`).then(r => r.json());

      } else {

        setMessage('❌ ' + (res.message || 'Lỗi'));        setStudents(s.students || []);        setStudents(s.students || []);

      }

    } catch (e) {        setCourses(c.courses || []);        setCourses(c.courses || []);
  ...
  const processPayment = async (e) => {        const bank = await fetch(`${API_BASE}/fee/bank-info`).then(r => r.json());        const bank = await fetch(`${API_BASE}/fee/bank-info`).then(r => r.json());

    e.preventDefault();

    if (!selectedInvoice) {        setInvoices(inv.invoices || []);        setInvoices(inv.invoices || []);

      setMessage('⚠️ Chọn hoá đơn');

      return;        if (bank.success && bank.bank) {        if (bank.success && bank.bank) {

    }

    setSubmitting(true);          setBankInfo(bank.bank);          setBankInfo(bank.bank);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/fee/invoices/${selectedInvoice.id}/payment`, {        }        }
        ...
        body: JSON.stringify({
          ...
          method: paymentMethod,        const inv = await fetch(`${API_BASE}/fee/invoices/all`).then(r => r.json());        const inv = await fetch(`${API_BASE}/fee/invoices/all`).then(r => r.json());
      ...
      if (res.success) {      }      }
        ...
        setPaymentMethod('cash');    } catch (e) {    } catch (e) {

        setPaymentNote('');

        setSelectedInvoice(null);      setMessage('❌ Lỗi: ' + e.message);      setMessage('❌ Lỗi: ' + e.message);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('currentUser');    e.preventDefault();    e.preventDefault();
  ...
  return (      setMessage('⚠️ Vui lòng chọn đầy đủ');      setMessage('⚠️ Vui lòng chọn đầy đủ');

    <div style={{

      width: '100%',      return;      return;
      ...
      <div style={{
        ...
        background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',    try {    try {

        color: 'white',

        padding: '40px 20px',      const res = await fetch(`${API_BASE}/fee/invoices`, {      const res = await fetch(`${API_BASE}/fee/invoices`, {

        boxShadow: '0 4px 20px rgba(0,0,0,0.1)',

        marginBottom: 30,        method: 'POST',        method: 'POST',

      }}>

        <div style={{        headers: { 'Content-Type': 'application/json' },        headers: { 'Content-Type': 'application/json' },

          maxWidth: 1200,

          margin: '0 auto',        body: JSON.stringify({        body: JSON.stringify({
          ...
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: 32, fontWeight: 700 }}>          amount: parseFloat(amount),          amount: parseFloat(amount),
