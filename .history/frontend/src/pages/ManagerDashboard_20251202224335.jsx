...
export default function ManagerDashboard() {
  ...
  async function saveAttendance() {
    ...
    try {
      const records = attendanceItems.map(it => ({
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ paddingTop: 0 }}>
      {/* Header gradient giống teacher - KHÔNG sticky, dài hơn */}
      <div className="w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-400 shadow-lg" style={{ padding: "40px 20px", marginBottom: 30 }}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white drop-shadow mb-2" style={{ margin: 0, fontSize: 32, fontWeight: 700 }}>Dashboard Quản lý</h1>
            <p className="text-white text-lg font-medium" style={{ margin: "8px 0 0 0", fontSize: 16, opacity: 0.9 }}>
              Xin chào, <span className="font-bold">{currentUser?.username || 'manager'}</span>
            </p>
          </div>
          <button
            onClick={handleLogout}
            className="mt-6 md:mt-0 px-6 py-2.5 border-2 border-white text-white rounded-lg font-semibold text-sm hover:bg-white hover:bg-opacity-30 transition shadow"
            style={{
      ...
      {/* Card chức năng */}
  <div className="max-w-5xl mx-auto -mt-12 mb-8 bg-white rounded-2xl shadow-xl p-8 relative z-10">
    {/* Tabs chức năng */}
    <div className="flex flex-wrap gap-3 mb-8 justify-center">
          <button className={`px-5 py-2 rounded-full font-semibold shadow-sm transition text-base ${tab==='list' ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-indigo-100'}`} onClick={()=>setTab('list')}>📋 Danh sách</button>
          <button className={`px-5 py-2 rounded-full font-semibold shadow-sm transition text-base ${tab==='attendance' ? 'bg-pink-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-pink-100'}`} onClick={()=>setTab('attendance')}>⏰ Chấm công</button>
          <button className={`px-5 py-2 rounded-full font-semibold shadow-sm transition text-base ${tab==='notify' ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-green-100'}`} onClick={()=>setTab('notify')}>📢 Thông báo</button>
        </div>
    ...
    {tab === 'list' && (
      <div>
        ...
        {/* Kết thúc nội dung card chức năng */}
      </div>
    </div>
  );
}
