...
function formatDate(dateStr) {
  ...
  // Nếu backend trả "YYYY-MM-DD" hoặc "YYYY-MM-DDTHH:mm:ss..."
  const m = s.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (m) {
    const year = m[1];
    const month = parseInt(m[2], 10); // 01 -> 1
    const day = parseInt(m[3], 10);   // 09 -> 9
    return `${day}/${month}/${year}`; // 9/1/2026
  }

  // Các trường hợp khác fallback dùng Date
  const d = new Date(s);
