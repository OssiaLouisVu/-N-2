...
export default function AddStudentPanel({ onGlobalMessage, onRefreshAll, refreshToken }) {
  ...
  const handleSubmit = async () => {
    ...
    if (providedId && String(providedId).trim() !== '') {
      ...
      if (Number.isInteger(pid) && pid > 0) payload.id = pid;
    }
    ...
    try {
      ...
      // after creation, optionally auto-assign to class to generate schedules
      if (!editingId && data && data.success && autoClassId && String(autoClassId).trim() !== '') {
        try {
          const classIdNum = Number(autoClassId);
          if (Number.isInteger(classIdNum) && classIdNum > 0) {
            const assignRes = await assignStudentToClass(classIdNum, data.id || payload.id);
            if (!assignRes || !assignRes.success) {
              showMessage((assignRes && assignRes.message) || 'Tạo học viên xong nhưng gán lớp thất bại.');
            } else {
              showMessage((data.username ? `Đã lưu và gán lớp. Tài khoản: ${data.username}` : 'Đã lưu và gán lớp.'));
            }
          }
        } catch (e) {
