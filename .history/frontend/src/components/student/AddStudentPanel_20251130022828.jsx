...
export default function AddStudentPanel({ onGlobalMessage, onRefreshAll, refreshToken }) {
  ...
  const handleSubmitNew = async (e) => {
    ...
    try {
      ...
      if (data && data.success) {
        // If backend returned credentials, show them to staff
        if (data.username && data.tempPassword) {
          const msg = `Đã thêm học viên. Tài khoản: ${data.username} / Mật khẩu tạm: ${data.tempPassword}`;
          setLocalMessage(msg);
          onGlobalMessage && onGlobalMessage(msg);
        } else {
