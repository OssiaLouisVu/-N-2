...
export default function OngoingStudentsPanel({
  ...
  const loadActiveStudents = async () => {
    try {
      ...
      if (!data.success) {
        showMessage(data.message || "Lỗi server khi tải học viên đang học.");
        return;
      }
  ...
  const handleMarkCompleted = async (st) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/students/${st.id}`, {
        method: "PUT",
      ...
      const data = await res.json();
      if (!data.success) {
        showMessage(data.message || "Lỗi server khi chuyển trạng thái.");
        return;
      }
