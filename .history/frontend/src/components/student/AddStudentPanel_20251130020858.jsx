...
export default function AddStudentPanel({ onGlobalMessage, onRefreshAll, refreshToken }) {
  ...
  // Add-new form (bottom)
  const [form, setForm] = useState({ full_name: "", phone: "", email: "", level: "", note: "" });
  ...
  const handleSubmitNew = async (e) => {
    ...
    try {
      ...
      if (data && data.success) {
        onGlobalMessage && onGlobalMessage("Thêm học viên mới thành công!");
        setForm({ full_name: "", phone: "", email: "", level: "", note: "" });
        onRefreshAll && onRefreshAll();
        await loadNewStudents();
      } else {
        onGlobalMessage && onGlobalMessage("Lỗi: " + ((data && data.message) || "Không thể thêm học viên"));
      }
    } catch (err) {
      onGlobalMessage && onGlobalMessage("Lỗi: Không thể thêm học viên");
    }
    setLoading(false);
  };
    ...
    <>
      ...
      <section style={{ borderRadius: 20, border: "1px solid #f3f3f3", padding: 20, marginBottom: 32, background: "#fff" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
          <button type="button" onClick={handleNewForm} style={{ padding: "8px 16px", borderRadius: 999, border: "1px solid #ddd", backgroundColor: "#fff", cursor: "pointer", fontSize: 13 }}>
            Làm mới form
          </button>

          <button type="button" onClick={handleSubmit} style={{ padding: "10px 24px", borderRadius: 999, border: "none", background: "linear-gradient(135deg, #ff7a7a 0%, #ff4d88 100%)", color: "#fff", cursor: "pointer", fontWeight: 600y }}>
      ...
      <section style={{ borderRadius: 20, border: "1px solid #f3f3f3", padding: 20, marginBottom: 32, background: "#fff" }}>
        ...
        <p style={{ fontSize: 13, color: "#555", marginBottom: 16 }}>Nhập thông tin học viên mới và nhấn nút <b>Thêm học viên mới</b> để lưu. Học viên sẽ nhận được email thông báo và hướng dẫn nhập học.</p>

        <form onSubmit={handleSubmitNew} style={{ marginBottom: 24 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Họ tên học viên *</label>
            <input name="full_name" value={form.full_name} onChange={handleChange} placeholder="VD: Nguyễn Văn A" required style={{ marginTop: 6, width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e0e0e0", outline: "none" }} />
          </div>

          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Email</label>
            <input name="email" value={form.email} onChange={handleChange} placeholder="vd: student@example.com" style={{ marginTop: 6, width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e0e0e0", outline: "none" }} />
          </div>

          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Số điện thoại</label>
            <input name="phone" value={form.phone} onChange={handleChange} placeholder="VD: 0987..." required style={{ marginTop: 6, width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e0e0e0", outline: "none" }} />
          </div>

          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Level hiện tại</label>
            <input name="level" value={form.level} onChange={handleChange} placeholder="VD: HSK1, HSK2..." style={{ marginTop: 6, width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e0e0e0", outline: "none" }} />
          </div>

          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Ghi chú</label>
            <textarea name="note" value={form.note} onChange={handleChange} placeholder="VD: nguồn Facebook, bạn giới thiệu, nhu cầu học..." rows={3} style={{ marginTop: 6, width: "100%", padding: "10px 12px", borderRadius: 12, border: "1px solid #e0e0e0", outline: "none", resize: "vertical" }} />
          </div>

          <button type="submit" disabled={loading} style={{ padding: "10px 24px", borderRadius: 999, border: "none", background: "linear-gradient(135deg, #4caf50 0%, #8bc34a 100%)", color: "#fff", cursor: "pointer", fontWeight: 600 }}>{loading ? "Đang thêm học viên..." : "Thêm học viên mới"}</button>
        </form>
      </section>
    </>
  );
