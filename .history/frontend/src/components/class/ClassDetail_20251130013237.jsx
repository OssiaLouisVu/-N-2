...
export default function ClassDetail({ classId, onDone = () => {} }) {
  ...
  useEffect(() => { loadNewStudents(); }, [classId, /* listen to prop if supplied */]);

  async function handleAssign() {
    if (!selectedStudentId) return alert('Chọn học viên để gán');
    if (!confirm('Gán học viên vào lớp và chuyển sang ACTIVE?')) return;
    try {
      const res = await assignStudentToClass(classId, Number(selectedStudentId));
      if (!res || !res.success) {
