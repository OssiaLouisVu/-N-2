...
export default function ClassList({ classes = [], refresh = () => {}, onSelectClass = () => {} }) {
    ...
    <table className="class-list" style={{ width: '100%', borderCollapse: 'collapse' }}>
      ...
      <tbody>
        {classes.map((c) => (
            ...
            <td>
              <button onClick={() => onSelectClass(c.id)}>Chi tiết</button>
              <button onClick={() => navigator.clipboard.writeText(String(c.id))} style={{ marginLeft: 8 }}>Sao chép ID</button>
