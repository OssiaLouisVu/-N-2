...
export default function ClassDetail({ classId, refreshToken = 0, onDone = () => {} }) {
  ...
  async function handleAssign() {
    ...
    try {
      ...
      const res = await assignStudentToClass(classId, Number(selectedStudentId), dates.length ? dates : undefined, sessionTime);
      if (!res || !res.success) {
    ...
    <div style={{ border: '1px solid #eee', padding: 12, borderRadius: 12, background: '#fafafa' }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <label style={{ fontSize: 13, marginRight: 8 }}>Giờ học chung cho các ngày đã nhập</label>
          <input type="time" value={sessionTime} onChange={(e) => setSessionTime(e.target.value)} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #ddd' }} />
          <div style={{ fontSize: 12, color: '#666' }}>Giờ sẽ được áp dụng cho tất cả ngày (ví dụ 09:00)</div>
        </div>
