...
export default function ClassDetail({ classId, refreshToken = 0, onDone = () => {} }) {
  ...
  async function handleCreateSchedules() {
    ...
    try {
      // allow creating class-level schedules when no student selected
      const sid = selectedStudentId ? Number(selectedStudentId) : undefined;
      const res = await createSchedulesForClass(classId, sid, dates, sessionStart || null, sessionEnd || null);
      if (!res || !res.success) return setMessage((res && res.message) || 'Tạo lịch thất bại');
      setMessage(sid ? 'Đã tạo lịch cho học viên' : 'Đã tạo lịch (cấp lớp) và cập nhật cho học viên hiện có');
      // reload schedules for selected student if any
      if (selectedStudentId) {
        const r2 = await getSchedules({ classId, studentId: selectedStudentId });
        if (r2 && r2.schedules) setStudentSchedules(r2.schedules);
      }
      try { window.dispatchEvent(new CustomEvent('studentScheduleUpdated', { detail: { studentId: sid ? Number(sid) : null, classId } })); } catch (e) {}
    } catch (e) {
      console.error(e);
            ...
            <tbody>
              {studentSchedules.map(s => (
                <tr key={s.id}>
                  ...
                  <td style={{ padding: 6 }}>
                    <button onClick={() => handleDeleteSchedule(s.id)} style={{ padding: '6px 8px', marginRight: 8 }}>Xóa</button>
                    {editingScheduleId === s.id ? (
                      <>
                        <button onClick={async () => {
                          // validation
                          if (!editDate) return alert('Chọn ngày hợp lệ');
                          if (!editTime) return alert('Chọn giờ bắt đầu hợp lệ');
                          const metaObj = {};
                          if (editEnd) metaObj.end = editEnd;
                          await handleUpdateSchedule(s.id, editDate, editTime, metaObj);
                          setEditingScheduleId(null);
                        }} style={{ padding: '6px 8px', marginRight: 8 }}>Lưu</button>
                        <button onClick={() => setEditingScheduleId(null)} style={{ padding: '6px 8px' }}>Hủy</button>
                      </>
                    ) : (
                      <button onClick={() => {
                        // enter edit mode
                        const parts = (s.scheduled_at || '').split(' ');
                        setEditDate(parts[0] || '');
                        setEditTime(parts[1] ? parts[1].slice(0,5) : (sessionStart || '09:00'));
                        try {
                          const metaObj = s.meta ? (typeof s.meta === 'string' ? JSON.parse(s.meta) : s.meta) : {};
                          setEditEnd(metaObj.end || sessionEnd || '');
                        } catch (e) {
                          setEditEnd(sessionEnd || '');
                        }
                        setEditingScheduleId(s.id);
                      }} style={{ padding: '6px 8px' }}>Sửa</button>
                    )}
                  </td>
                </tr>
              ))}
