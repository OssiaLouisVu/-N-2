...
export default function ClassList({ classes = [], refresh = () => {}, onEdit = () => {}, onSelectClass = () => {}, readOnly = false }) {
        ...
        {classes.map((c) => (
          <tr key={c.id} style={{ borderTop: '1px solid #eee' }}>
            ...
            <td>
              ...
              <button onClick={() => onSelectClass(c.id)} style={{ marginLeft: 8 }}>Chi tiết</button>
              {!readOnly && <button onClick={() => handleDelete(c.id)} style={{ marginLeft: 8 }}>Xoá</button>}
            </td>
          </tr>
        ))}
      </tbody>
