              ...
              {studentSchedules.map(s => (
                <tr key={s.id}>
                  ...
                  <td style={{ padding: 6 }}>
                    ...
                    <button onClick={() => {
                      const newDate = prompt('Nhập ngày mới (YYYY-MM-DD)', (s.scheduled_at || '').split(' ')[0] || '');
                      if (newDate === null) return;
                      const newTime = prompt('Nhập giờ bắt đầu mới (HH:MM)', (s.scheduled_at || '').split(' ')[1] || (sessionStart || '09:00'));
                      if (newTime === null) return;
                      // optionally keep meta end
                      let metaObj = {};
                      try { metaObj = s.meta ? JSON.parse(s.meta) : {}; } catch (e) { metaObj = {}; }
                      if (sessionEnd) metaObj.end = sessionEnd;
                      handleUpdateSchedule(s.id, newDate, newTime, metaObj);
