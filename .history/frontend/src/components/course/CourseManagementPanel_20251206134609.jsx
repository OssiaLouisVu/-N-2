  ...
  const fetchHistory = async (courseId) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/courses/${courseId}/history`);
    ...
    <div>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                      ...
                      <span style={{
                        ...
                        fontWeight: 600,
                        background: 
                          course.status === 'ACTIVE' ? '#d4edda' : '#f8d7da',
                        color:
                          course.status === 'ACTIVE' ? '#155724' : '#721c24'
                      }}>
                        {course.status === 'ACTIVE' ? '✅ Hoạt động' : '🍂 Đã lưu trữ'}
                      </span>
                      {course.level && (
                        <span style={{
                          padding: '2px 8px',
      ...
      {detailCourse && (
        <div style={{
          ...
          <div style={{ background: '#fff', borderRadius: 16, minWidth: 600, maxWidth: 800, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: 32, position: 'relative' }}>
            <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>
              📚 {detailCourse.name}
            </h2>
            <div style={{ color: '#666', marginBottom: 12 }}>
              Mã: <b>{detailCourse.course_code}</b> | Level: <b>{detailCourse.level}</b> | Học phí: <b>{Number(detailCourse.tuition_fee).toLocaleString('vi-VN')} đ</b>
            </div>
