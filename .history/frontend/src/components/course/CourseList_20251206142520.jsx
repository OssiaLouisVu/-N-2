...
import { deleteCourse, archiveCourse, updateCourse } from '../../api/courseApi';
  ...
  async function handleActivate(id) {
    if (!confirm('Chuyển khóa học này sang trạng thái Hoạt động?')) return;
    ...
    try {
      // Gửi toàn bộ object khóa học, chỉ thay đổi status
      const updatedCourse = {
        ...course,
        status: 'ACTIVE',
        name: course.name || '',
        ...
        hours_per_session: course.hours_per_session || 0,
      ...
      if (res.success) {
        ...
        // Không gọi refresh() ở đây vì tab sẽ tự động chuyển và loadCourses sẽ được gọi ở cha
      } else {
