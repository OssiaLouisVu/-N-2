...
export default function InactiveInstructorsPanel({
  ...
  const handleReactivate = async (instructor) => {
    const confirmMsg = `🔄 Kích hoạt lại giảng viên "${instructor.full_name}"?\n\n` +
      `Giảng viên sẽ chuyển sang trạng thái ACTIVE và có thể được phân công dạy lớp.`;
    ...
    try {
      ...
      showMessage(`✅ Đã kích hoạt lại giảng viên "${instructor.full_name}".`);
      await loadInactiveInstructors();
  ...
  const handleDelete = async (instructor) => {
    const confirmMsg = `⚠️ XÓA VĨNH VIỄN giảng viên "${instructor.full_name}"?\n\n` +
      ...
      `- Tài khoản giảng viên sẽ bị xóa\n` +
      `- Lịch sử dạy học sẽ bị mất\n\n` +
