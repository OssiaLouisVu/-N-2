...
export default function AddInstructorPanel({
  ...
  const handleDelete = async (instructor) => {
    if (!window.confirm(`⚠️ XÓA VĨNH VIỄN giảng viên "${instructor.full_name}"?\n\nDữ liệu sẽ bị xóa khỏi database`)) return;
    ...
    try {
      ...
      showMessage(`✅ Đã xóa giảng viên "${instructor.full_name}".`);
      await loadNewInstructors();
      if (onRefreshAll) onRefreshAll();
    } catch (err) {
      console.error(err);
