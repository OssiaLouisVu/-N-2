...
export default function ActiveInstructorsPanel({
  ...
  const handleSetOnLeave = async (instructor) => {
    ...
    try {
      ...
      showMessage(`✅ Đã chuyển "${instructor.full_name}" sang trạng thái NGHỈ PHÉP.`);
      await loadActiveInstructors();
  ...
  const handleSetInactive = async (instructor) => {
    const confirmMsg = `⛔ Đặt giảng viên "${instructor.full_name}" sang trạng thái KHÔNG HOẠT ĐỘNG?\n\n` +
      `Giảng viên sẽ chuyển sang danh sách không hoạt động.`;
    ...
    try {
      ...
      showMessage(`✅ Đã chuyển "${instructor.full_name}" sang trạng thái KHÔNG HOẠT ĐỘNG.`);
      await loadActiveInstructors();
      if (onRefreshAll) onRefreshAll();
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (instructor) => {
    const confirmMsg = `⚠️ XÓA VĨNH VIỄN giảng viên "${instructor.full_name}"?\n\n` +
      ...
      `- Tài khoản giảng viên sẽ bị vô hiệu hóa\n` +
    ...
    if (typedName !== instructor.full_name) {
