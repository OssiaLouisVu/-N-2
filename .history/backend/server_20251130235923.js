...
app.get("/api/students/:username/schedule", async(req, res) => {
    ...
    try {
        ...
        // If latestOnly=true, keep only upcoming items and limit to N (default 5)
        if (latestOnly) {
            const now = new Date();
            const upcoming = normalized.filter((it) => {
                try {
        ...
        return res.json({ success: true, schedule: normalized });
    } catch (err) {
