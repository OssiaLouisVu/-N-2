...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const [students] = await db.query(
      `SELECT s.* FROM class_students cs JOIN students s ON cs.student_id = s.id WHERE cs.class_id = ?`,
      [id]
    );
...
router.post('/:id/assign', async (req, res) => {
  ...
  try {
    ...
    const [[cls]] = await conn.query('SELECT capacity FROM classes WHERE id = ?', [classId]);
