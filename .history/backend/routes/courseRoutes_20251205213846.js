  ...
  try {
    ...
    await db.query(
      `INSERT INTO course_history (course_id, action, changed_by)
       VALUES ($1, 'CREATE', $2)`,
      [result.rows[0].id, req.body.created_by || 1]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const oldData = await db.query('SELECT * FROM courses WHERE id = $1', [id]);
    ...
    const result = await db.query(
      `UPDATE courses SET
        name = $1, level = $2, short_description = $3, detailed_description = $4,
        duration_weeks = $5, sessions_per_week = $6, hours_per_session = $7,
        tuition_fee = $8, requirements = $9, objectives = $10, status = $11,
        updated_at = NOW()
      WHERE id = $12
      RETURNING *`,
      [
