    ...
    try {
        ...
        if (!studentId) {
            ...
            for (const raw of sessionDates) {
                ...
                metaObj.tz = 'Asia/Ho_Chi_Minh'; // thêm thông tin múi giờ để frontend dùng nếu cần
                const meta = JSON.stringify(metaObj);
                ...
                try {
                    ...
                    if (existingCS && existingCS.length > 0) {
                        // Kiểm tra nếu đã có lịch trùng hoàn toàn (cùng start & end) -> skip không tạo lại
                        const hasIdentical = existingCS.some(row => {
                            let parsed = null;
                            try { parsed = row.meta ? JSON.parse(row.meta) : null; } catch (e) { parsed = null; }
                            const existingStart = parsed && parsed.start ? parsed.start : (row.scheduled_at ? (new Date(row.scheduled_at)).toISOString().slice(11,16) : null);
                            const existingEnd = parsed && parsed.end ? parsed.end : null;
                            return existingStart === start && existingEnd === sessionTimeEnd;
                        });
                        if (hasIdentical) {
                            skipped.push({ date: dateStr, start, end: sessionTimeEnd });
                            continue; // bỏ qua vòng lặp này, không xoá / không tạo lại
                        }
