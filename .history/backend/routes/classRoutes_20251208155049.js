...
router.post('/:id/assign-instructor', async(req, res) => {
    ...
    try {
        ...
        const [[cls]] = await conn.query(
            'SELECT id, class_name, start_date, end_date FROM classes WHERE id = ?',
            [classId]
        );
        if (!cls) {
        ...
        const [[instructor]] = await conn.query(
            'SELECT id, full_name, status FROM instructors WHERE id = ?',
            [instructorId]
        );
        if (!instructor) {
        ...
        const [classSchedules] = await conn.query(
            'SELECT scheduled_at, meta FROM class_schedules WHERE class_id = ? ORDER BY scheduled_at',
            [classId]
        );
        ...
        const [instructorClasses] = await conn.query(
            `SELECT DISTINCT ct.class_id 
             ...
             WHERE ct.teacher_id = ? AND ct.class_id != ?`,
        ...
        if (instructorClasses.length > 0) {
            ...
            const [otherSchedules] = await conn.query(
                `SELECT cs.scheduled_at, cs.meta, c.class_name
        ...
        await conn.query(
            'INSERT INTO class_teachers (class_id, teacher_id, role, assigned_at) VALUES (?, ?, ?, NOW())',
        ...
        // 8. Lưu lịch sử gán lớp
        await conn.query(
            'INSERT INTO instructor_class_history (instructor_id, class_id, action, created_at) VALUES (?, ?, ?, NOW())',
            [instructorId, classId, 'ASSIGNED']
        );
        ...
        res.json({
            success: true,
            message: `Đã gán giảng viên "${instructor.full_name}" vào lớp "${cls.class_name}"${instructor.status === 'NEW' ? ' và chuyển sang trạng thái ACTIVE' : ''}`
        });
