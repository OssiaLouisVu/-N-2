...
async function fetchFilteredEmployees({ role = 'ALL', search = '', active = 'true' }) {
  ...
  if (role && role !== 'ALL') { where.push('(COALESCE(u.role, e.role) = ?)'); params.push(role); }
...
router.get('/monthly', async (req, res) => {
  ...
  try {
    ...
    const [[{ lastDay }]] = await db.query('SELECT LAST_DAY(?) AS lastDay', [firstDay]);
...
router.post('/notify/daily', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.execute(
      `SELECT employee_id, status, note FROM attendance WHERE date = ?`,
      [date]
    );
    const byEmp = new Map();
...
router.post('/notify/monthly', async (req, res) => {
  ...
  try {
    ...
    for (const { e, present, leave, sick, absent } of map.values()) {
