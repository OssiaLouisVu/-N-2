...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    // Chỉ cho phép update status nếu chỉ truyền status
      ...
      if (oldRows[0].status !== req.body.status) {
        await db.query(
          `INSERT INTO course_history (course_id, action, field_changed, old_value, new_value, reason, changed_by)
           VALUES (?, 'UPDATE', 'status', ?, ?, 'Chuyển trạng thái', 1)`,
          [id, oldRows[0].status, req.body.status]
        );
      }
      const [rows] = await db.query('SELECT * FROM courses WHERE id = ?', [id]);
      return res.json({ success: true, course: rows[0] });
    }
    // ...các trường hợp update đầy đủ khác giữ nguyên như cũ...
  } catch (error) {
...
router.put('/:id/archive', async (req, res) => {
  try {
    ...
    await db.query(
      `INSERT INTO course_history (course_id, action, field_changed, old_value, new_value, reason, changed_by)
       VALUES (?, 'UPDATE', 'status', ?, 'ARCHIVED', 'Kết thúc khóa học', ?)`,
      [id, courseRows[0].status, changed_by || 1]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Kiểm tra khoá học có đang được dùng không
    const [classRows] = await db.query('SELECT COUNT(*) as count FROM classes WHERE course_id = ?', [id]);
    if (parseInt(classRows[0].count) > 0) {
      return res.status(400).json({ 
        success: false, 
...
router.get('/:id/lessons', async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      'SELECT * FROM course_lessons WHERE course_id = ? ORDER BY lesson_number',
      [id]
    );
...
router.get('/lessons/:id/sub-lessons', async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await db.query(
      'SELECT * FROM course_sub_lessons WHERE lesson_id = ? ORDER BY sub_lesson_number',
      [id]
    );
