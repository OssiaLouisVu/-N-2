    ...
    let sql = `
        SELECT
            ...
            (SELECT COUNT(*) FROM class_teachers ct 
             ...
             WHERE ct.teacher_id = i.id AND c.status = 'ACTIVE') as active_classes_count
        FROM instructors i
        WHERE 1 = 1
    `;
    const params = [];
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        console.error('Error INSERT instructor and create user:', err);
        return res.status(500).json({
            success: false,
            message: 'Lỗi server khi thêm giảng viên.',
        });
    } finally {
...
router.get("/:id/statistics", async (req, res) => {
    ...
    try {
        ...
        const [completedClasses] = await db.query(`
            ...
            FROM class_teachers ct
            INNER JOIN classes c ON ct.class_id = c.id
            WHERE ct.teacher_id = ? AND c.status = 'COMPLETED'
        `, [id]);
