...
router.post('/', async (req, res) => {
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO classes (name, teacher_id, capacity, level, start_date, end_date, note)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [name, teacher_id || null, capacity || 20, level || null, start_date || null, end_date || null, note || null]
        );
        res.json({ success: true, id: result.insertId });
    } catch (err) {
        console.error('POST /api/classes error', err);
...
router.post('/:id/assign', async (req, res) => {
    ...
    try {
        ...
        const [[cls]] = await conn.query('SELECT id, capacity, start_date, end_date FROM classes WHERE id = ?', [classId]);
        if (!cls) throw { code: 'NOT_FOUND', message: 'Class not found' };

        const [[cnt]] = await conn.query('SELECT COUNT(*) AS cnt FROM class_students WHERE class_id = ? AND status = "ACTIVE"', [classId]);
...
router.delete('/schedules/:id', async (req, res) => {
    ...
    try {
        ...
        const [[origSched]] = await db.query('SELECT * FROM schedules WHERE id = ?', [scheduleId]);
        if (origSched) {
            await db.query('DELETE FROM schedules WHERE id = ?', [scheduleId]);
        ...
        const [[orig]] = await db.query('SELECT * FROM class_schedules WHERE id = ?', [scheduleId]);
