...
router.post("/:id/create-account", async (req, res) => {
  ...
  try {
    ...
    const [empRows] = await db.execute(
      `SELECT id, user_id, role FROM employees WHERE id = ?`,
      [employeeId]
    );
    ...
    const employeeRole = empRows[0].role || 'STAFF';
    ...
    res.json({ success: true, message: "Tạo tài khoản thành công", userId: newUserId });
  } catch (e) {
    res.status(500).json({ success: false, message: e.message });
  }
});
