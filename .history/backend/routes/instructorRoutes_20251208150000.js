...
router.delete("/:id", async(req, res) => {
    ...
    try {
        ...
        // 4. SOFT DELETE: Đánh dấu inactive thay vì xóa hẳn
        const [result] = await db.query(
            "UPDATE instructors SET active = FALSE WHERE id = ?", 
            [instructorId]
        );
        ...
        console.log(`✅ Đã vô hiệu hóa giảng viên ID=${instructorId} (${instructor.full_name})`);
