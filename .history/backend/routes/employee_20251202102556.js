...
router.put("/:id", async (req, res) => {
  ...
  try {
    // 1) Cập nhật thông tin nhân viên (dùng query để tránh lỗi Malformed packet ở một số phiên bản MariaDB)
    await db.query(
    ...
    if (role) {
      ...
      if (rows && rows.length && rows[0].user_id) {
        await db.query(`UPDATE users SET role = ? WHERE id = ?`, [role, rows[0].user_id]);
      }
    }
