...
// Optional body: { studentId, sessionDates: ['2025-12-01','2025-12-02', ...] }
router.post('/:id/assign', async(req, res) => {
    const classId = parseInt(req.params.id, 10);
    const { studentId, sessionDates } = req.body || {};
    let conn;
    try {
        ...
        // If the caller provided explicit `sessionDates` (array of YYYY-MM-DD), create class_sessions for those dates
        // (two sessions per day: 09:00-11:00 and 14:00-16:00) if none exist yet. Otherwise, if class has start_date/end_date
        // and no sessions exist, fall back to generating sessions for the date range.
        try {
            ...
            if (sessCnt === 0) {
                if (Array.isArray(sessionDates) && sessionDates.length > 0) {
                    // sanitize and insert sessions for provided dates
                    for (const raw of sessionDates) {
                        if (!raw) continue;
                        // Expect date in YYYY-MM-DD — do a simple check
                ...
                } else if (cls && cls.start_date && cls.end_date) {
            ...
            // Determine date window to fetch sessions for schedule insertion
            let dateFrom = null;
            let dateTo = null;
            if (Array.isArray(sessionDates) && sessionDates.length > 0) {
                const sorted = sessionDates.filter(Boolean).map(s => s).sort();
                dateFrom = sorted[0];
