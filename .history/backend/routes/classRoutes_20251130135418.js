    ...
    try {
        ...
        if (!studentId) {
            ...
            for (const raw of sessionDates) {
                ...
                // Tìm lịch cùng ngày để kiểm tra trùng (dùng JSON_EXTRACT nếu meta là JSON, fallback LIKE + DATE)
                let existingCS = [];
                try {
                    [existingCS] = await conn.query(
                        `SELECT id, scheduled_at, meta FROM class_schedules WHERE class_id = ? AND (
                           (scheduled_at IS NOT NULL AND DATE(scheduled_at) = ?) OR
                           (meta LIKE ?)
                        )`,
                        [classId, dateStr, `%"providedSessionDate":"${dateStr}"%`]
                    );
                } catch (e) {
                    console.warn('Query existing class_schedules failed', e.message);
                }
                ...
                if (existingCS && existingCS.length > 0) {
                    for (const row of existingCS) {
                        let parsed = null;
                        try { parsed = row.meta ? JSON.parse(row.meta) : null; } catch (_) { parsed = null; }
                        const existingDate = parsed && parsed.providedSessionDate ? parsed.providedSessionDate : (row.scheduled_at ? new Date(row.scheduled_at).toISOString().slice(0,10) : null);
                ...
                // Nếu có lịch cùng ngày nhưng khác start/end -> xoá cũ để thay thế
                if (existingCS && existingCS.length > 0) {
