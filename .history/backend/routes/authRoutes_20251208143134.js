...
router.post("/forgot-password", async(req, res) => {
         ...
         WHERE s.email = ? AND u.role = 'STUDENT'`, [email.trim()]
                    );
                    users = stdRows || [];
                } else if (role === 'INSTRUCTOR') {
                    // Tìm trong bảng instructors
                    const [instRows] = await db.execute(
                        `SELECT i.id, i.full_name AS name, i.email, i.user_id, u.username
         FROM instructors i
         LEFT JOIN users u ON u.id = i.user_id
         WHERE i.email = ? AND i.active = TRUE AND i.user_id IS NOT NULL AND u.role = 'INSTRUCTOR'`, [email.trim()]
                    );
                    users = instRows || [];
         ...
         WHERE e.email = ? AND e.active = TRUE AND e.user_id IS NOT NULL AND u.role = 'TEACHER'`, [email.trim()]
                    );
           ...
           WHERE u.email = ? AND u.role = 'TEACHER'`, [email.trim()]
                        );
                        users = userRows || [];
                    } else {
                        users = empRows || [];
         ...
         FROM employees e
         LEFT JOIN users u ON u.id = e.user_id
         WHERE e.email = ? AND e.active = TRUE AND e.user_id IS NOT NULL AND u.role = ?`, [email.trim(), role]
                    );
                    users = empRows || [];
                }
