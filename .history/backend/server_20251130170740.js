...
app.get("/api/students/:username/schedule", async(req, res) => {
    ...
    try {
        ...
        // Merge duplicates (same classId + date + timeStart). Prefer 'schedule' source (has richer meta like room).
        const mergedMap = new Map();
        for (const item of normalized) {
            const key = `${item.classId || 'NA'}|${item.date || 'NA'}|${item.timeStart || 'NA'}`;
            if (!mergedMap.has(key)) {
                mergedMap.set(key, item);
            } else {
                const existing = mergedMap.get(key);
                // Prefer per-student schedule over class_session,
                // or any item that contains a non-empty room
                const existingScore = (existing.source === 'schedule' ? 2 : 1) + (existing.room ? 1 : 0);
                const candidateScore = (item.source === 'schedule' ? 2 : 1) + (item.room ? 1 : 0);
        ...
        return res.json({ success: true, schedule: merged });
    } catch (err) {
...
app.get("/api/teacher/mock-exam-results", async(req, res) => {
    ...
    try {
      ...
      SELECT
        ...
        if (latestOnly) {
            const now = new Date();
            const upcoming = merged.filter((it) => {
                try {
                    const d = new Date(`${it.date}T${(it.timeStart || '00:00')}:00Z`);
                    return d.getTime() >= now.getTime();
                } catch (e) { return false; }
            });
            return res.json({ success: true, schedule: upcoming.slice(0, limit) });
        }
        
        return res.json({ success: true, schedule: merged.slice(0, limit) });
