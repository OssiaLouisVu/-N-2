...
// Lấy danh sách ca thi mà học viên đã đăng ký + điểm (nếu có)
// Query param (tuỳ chọn):
//   examName: lọc theo tên kỳ thi (LIKE %...%)
//   date:     lọc theo ngày thi YYYY-MM-DD (theo ca thi)
// Frontend dùng trường "status" để lọc: Đã thi / Đang thi (hôm nay) / Sắp thi
app.get("/api/teacher/mock-exam-results", async(req, res) => {
    ...
    `;
        ...
        // lọc theo ngày thi của ca thi (nếu có)
        if (date) {
            sql += " AND s.date = ?";
            params.push(date);
        }
        ...
        // FINISHED: examDate < hôm nay
        // ONGOING:  examDate = hôm nay
        // UPCOMING: examDate > hôm nay
        ...
        const results = rows.map((r) => {
            ...
            return {
                // dùng registrationId làm id cho dễ key ở frontend
                id: r.registrationId,
