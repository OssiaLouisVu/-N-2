// API cập nhật ngày bắt đầu/kết thúc cho lớp
app.patch("/api/classes/:id", async (req, res) => {
    ...
    try {
        await db.query(
            `UPDATE classes SET start_date = ?, end_date = ? WHERE id = ?`,
...
app.get("/api/teacher/:username/classes", async (req, res) => {
    ...
    try {
        ...
        // 2. Lấy danh sách lớp mà giảng viên đang dạy với status
        ...
        return res.json({
            ...
            classes: classes.map(c => ({
                ...
                endDate: c.end_date,
                status: c.status // UPCOMING, ACTIVE, hoặc COMPLETED
            }))
        });
    } catch (err) {
