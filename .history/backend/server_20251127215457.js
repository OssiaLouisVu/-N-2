...
// ======================================================
// Dùng cho: Panel "Xem danh sách thi thử / điểm thi thử" (MockExamPanel)
...
app.get("/api/teacher/mock-exam-results", async(req, res) => {
    ...
    `;
        ...
        // Xác định trạng thái theo ngày của ca thi
        const todayStr = new Date().toISOString().split("T")[0];
        const today = new Date(todayStr);
        ...
        const results = rows.map((r) => {
            ...
            return {
                // nếu chưa có đăng ký vẫn tạo id dựa trên ca thi
                id: r.registrationId || `shift-${r.shiftId}`,
                shiftId: r.shiftId,
                examName: r.examName,
                date: r.examDate,
                startTime: r.startTime,
                endTime: r.endTime,
                room: r.room,
                // có thể null nếu chưa có học viên đăng ký / chưa có điểm
                username: r.username || null,
                score: r.score == null ? null : r.score,
                feedback: r.feedback == null ? null : r.feedback,
