...
app.get("/api/students/:username/schedule", async(req, res) => {
    ...
    try {
       ...
       ORDER BY date ASC`, [username]
        ...
        return res
            .status(500)
            .json({ success: false, message: "Lỗi server khi lấy lịch học" });
    }
});
...
// ======================================================
// 6.1) GIÁO VIÊN – XEM DANH SÁCH CÁC KỲ THI THỬ (NHÁNH 1)
// ======================================================
// Dùng cho: Xem danh sách kỳ thi đã / đang / sắp thi (kể cả khi chưa có ai đăng ký)
...
// ======================================================
// 6.2) GIÁO VIÊN – TÌM KIẾM ĐIỂM THI THỬ (NHÁNH 2)
// ======================================================
// Dùng cho: Xem danh sách điểm thi thử của học viên theo kỳ thi / ngày / trạng thái
...
// 6) GIÁO VIÊN – XEM DANH SÁCH ĐIỂM THI THỬ
// ======================================================
// 6) GIÁO VIÊN – THI THỬ (LỊCH + ĐIỂM)
// ======================================================
//
// Endpoint này làm 2 việc cho FE:
//  - Trả về tất cả các ca thi (kể cả chưa có học viên đăng ký).
//  - Trả về thông tin đăng ký + điểm (nếu có) cho từng học viên.
//
app.get("/api/teacher/mock-exam-results", async(req, res) => {
    const { examName, date } = req.query;

    try {
        // BẮT ĐẦU từ bảng ca thi, rồi LEFT JOIN sang đăng ký + kết quả
        let sql = `
      SELECT
        s.id         AS shiftId,
        s.exam_name  AS examName,
        ...
        res.id       AS resultId,
        res.score    AS score,
        res.feedback AS feedback
      FROM mock_exam_shifts s
      LEFT JOIN mock_exam_registrations r
        ON r.shift_id = s.id
      LEFT JOIN mock_exam_results res
        ON res.username = r.username
       AND res.exam_name = s.exam_name
       AND res.date = s.date
      WHERE 1 = 1
    `;
        ...
        // Lọc theo tên kỳ thi (nếu có)
        if (examName) {
            sql += " AND s.exam_name LIKE ?";
            params.push(`%${examName}%`);
        }

        // Lọc theo ngày thi (nếu có)
        if (date) {
            sql += " AND s.date = ?";
            params.push(date);
        }

        sql += " ORDER BY s.date DESC, s.exam_name ASC, r.username ASC";

        const [rows] = await db.execute(sql, params);

        // Xác định trạng thái kỳ thi theo ngày của CA THI
        const todayStr = new Date().toISOString().split("T")[0];
        const today = new Date(todayStr);
        ...
        const results = rows.map((r) => {
            ...
            return {
                // bản ghi theo "ca thi + (tùy chọn) học viên"
                id: r.registrationId || `shift-${r.shiftId}`, // nếu chưa có đăng ký thì vẫn có id
                shiftId: r.shiftId,
                ...
                username: r.username || null, // có thể null nếu chưa có HV đăng ký
                score: r.score ? ? null,
