...
app.get("/api/students/:username/schedule", async(req, res) => {
    ...
    try {
        ...
        // Only show schedules for ACTIVE classes (not COMPLETED)
        ...
        // Only show schedules for classes where student is still ACTIVE
        const [schedRows] = await db.execute(
            `SELECT s.scheduled_at, s.meta, s.class_id
                         FROM schedules s
                         JOIN class_students cls ON cls.class_id = s.class_id AND cls.student_id = s.student_id
                         WHERE s.student_id = ?
                             AND cls.status = 'ACTIVE'
        ...
        const normalized = [];
        const seen = new Map(); // dedup by classId+date+timeStart

        // For schedule rows, scheduled_at may be a full datetime (YYYY-MM-DD HH:MM:SS)
        // DEDUP: Only add unique sessions from schedules table
        for (const s of schedRows) {
            ...
            // Skip if already seen (duplicate in schedules table)
            if (seen.has(key)) {
                continue;
            }
            seen.set(key, true);
